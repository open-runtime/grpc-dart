...
class Http2StreamImpl extends TransportStream implements ClientTransportStream, ServerTransportStream {
  ...
  final ZoneBinaryCallback<ServerTransportStream, Http2StreamImpl, List<Header>> _pushStreamFun;
  ...
  /// responds to.
  @override
  ServerTransportStream push(List<Header> requestHeaders) => _pushStreamFun(this, requestHeaders);
...
class StreamHandler extends Object with TerminatableMixin, ClosableMixin {
  ...
  void onTerminated(Object? error) {
    _openStreams.values.toList().forEach((stream) => _closeStreamAbnormally(stream, error, propagateException: true));
    startClosing();
  }
  ...
  void processGoawayFrame(GoawayFrame frame) {
    ...
    var streamIds = _openStreams.keys.where((id) => id > lastStreamId && !_isPeerInitiatedStream(id)).toList();
  ...
  Http2StreamImpl newLocalStream() {
    return ensureNotTerminatedSync(() {
      ...
      if (MAX_STREAM_ID < nextStreamId) {
        throw StateError('Cannot create new streams, since a wrap around would happen.');
      }
  ...
  Http2StreamImpl _newStreamInternal(int streamId) {
    ...
    var outgoingStreamWindow = Window(initialSize: _peerSettings.initialWindowSize);

    var incomingStreamWindow = Window(initialSize: _localSettings.initialWindowSize);

    var windowOutHandler = OutgoingStreamWindowHandler(outgoingStreamWindow);

    var windowInHandler = IncomingWindowHandler.stream(_frameWriter, incomingStreamWindow, streamId);
    ...
    streamQueueIn.onCancel.then((_) {
      ...
      if (stream.state == StreamState.HalfClosedLocal) {
        stream.outgoingQueue.enqueueMessage(ResetStreamMessage(stream.id, ErrorCode.CANCEL));
      }
    });
  ...
  bool _canPush(Http2StreamImpl stream) {
    var openState = stream.state == StreamState.Open || stream.state == StreamState.HalfClosedRemote;
    var pushEnabled = _peerSettings.enablePush;
    return openState && pushEnabled && _canCreateNewStream() && !_ranOutOfStreamIds();
  }

  ServerTransportStream _push(Http2StreamImpl stream, List<Header> requestHeaders) {
    ...
    pushStream.incomingQueue.enqueueMessage(DataMessage(stream.id, const <int>[], true));

    _frameWriter.writePushPromiseFrame(stream.id, pushStream.id, requestHeaders);
  ...
  void _handleNewOutgoingMessage(Http2StreamImpl stream, StreamMessage msg) {
    if (stream.state == StreamState.Idle) {
      if (msg is! HeadersStreamMessage) {
        var exception = TransportException('The first message on a stream needs to be a headers frame.');
    ...
    if (stream.outgoingQueue.bufferIndicator.wouldBuffer && !stream._outgoingCSubscription.isPaused) {
  ...
  void processStreamFrame(ConnectionState connectionState, Frame frame) {
    try {
      ...
      _frameWriter.writeRstStreamFrame(exception.streamId, ErrorCode.STREAM_CLOSED);
      _closeStreamIdAbnormally(exception.streamId, exception);
    } on StreamException catch (exception) {
      _frameWriter.writeRstStreamFrame(exception.streamId, ErrorCode.INTERNAL_ERROR);
      _closeStreamIdAbnormally(exception.streamId, exception);
    }
  }

  void _processStreamFrameInternal(ConnectionState connectionState, Frame frame) {
    ...
    return ensureNotTerminatedSync(() {
      ...
      if (stream == null) {
        bool frameBelongsToIdleStream() {
          ...
          var isLocalStream = isServerStreamId == isServer;
          var isIdleStream = isLocalStream ? streamId >= nextStreamId : streamId > lastRemoteStreamId;
          return isIdleStream;
        }
        ...
        if (_isPeerInitiatedStream(frame.header.streamId)) {
          ...
          // retry it).
          _highestStreamIdReceived = max(_highestStreamIdReceived, frame.header.streamId);
        }
        ...
        if (frame is HeadersFrame) {
          ...
          if (frameBelongsToIdleStream()) {
            ...
            throw ProtocolException('Got a WINDOW_UPDATE_FRAME for an "idle" stream id.');
          } else {
          ...
          if (frameBelongsToIdleStream()) {
            ...
            throw ProtocolException('Got a RST_STREAM_FRAME for an "idle" stream id.');
          } else {
        ...
        if (frame is HeadersFrame) {
          ...
          throw ProtocolException('Unsupported frame type ${frame.runtimeType}.');
        }
      }
    });
  }
  ...
  void _handleHeadersFrame(Http2StreamImpl stream, HeadersFrame frame) {
    ...
    if (stream.state != StreamState.Open && stream.state != StreamState.HalfClosedLocal) {
      throw StreamClosedException(stream.id, 'Expected open state (was: ${stream.state}).');
    }
  ...
  void _handleRstFrame(Http2StreamImpl stream, RstStreamFrame frame) {
    ...
    var exception = StreamTransportException('Stream was terminated by peer (errorCode: ${frame.errorCode}).');
  ...
  void _sendHeaders(Http2StreamImpl stream, List<Header> headers, {bool endStream = false}) {
    if (stream.state != StreamState.Idle &&
    ...
    stream.outgoingQueue.enqueueMessage(HeadersMessage(stream.id, headers, endStream));
  ...
  void _sendData(Http2StreamImpl stream, List<int> data, {bool endStream = false}) {
    if (stream.state != StreamState.Open && stream.state != StreamState.HalfClosedRemote) {
      throw StateError('Open state expected (was: ${stream.state}).');
    }

    stream.outgoingQueue.enqueueMessage(DataMessage(stream.id, data, endStream));
  ...
  void _closeStreamIdAbnormally(int streamId, Exception exception, {bool propagateException = false}) {
    var stream = _openStreams[streamId];
    if (stream != null) {
      _closeStreamAbnormally(stream, exception, propagateException: propagateException);
    }
  }

  void _closeStreamAbnormally(Http2StreamImpl stream, Object? exception, {bool propagateException = false}) {
    incomingQueue.removeStreamMessageQueue(stream.id);
  ...
  void _changeState(Http2StreamImpl stream, StreamState to) {
    ...
    assert(
      (from == StreamState.Idle && to == StreamState.ReservedLocal) ||
          ...
          (from == StreamState.ReservedLocal && to == StreamState.HalfClosedRemote) ||
          (from == StreamState.ReservedLocal && to == StreamState.Closed) ||
          (from == StreamState.ReservedRemote && to == StreamState.Closed) ||
          (from == StreamState.ReservedRemote && to == StreamState.HalfClosedLocal) ||
    ...
    if (_didInitiateStream(stream)) {
      ...
      switch (stream.state) {
        ...
        case StreamState.Idle:
          if (to == StreamState.Open || to == StreamState.HalfClosedLocal || to == StreamState.HalfClosedRemote) {
  ...
  static Exception _throwStreamClosedException(int streamId) => StreamClosedException(
    streamId,
