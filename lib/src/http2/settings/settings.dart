...
class SettingsHandler extends Object with TerminatableMixin {
  ...
  final _onInitialWindowSizeChangeController = StreamController<int>.broadcast(sync: true);

  /// Events are fired when a SettingsFrame changes the initial size
  /// of stream windows.
  Stream<int> get onInitialWindowSizeChange => _onInitialWindowSizeChangeController.stream;

  SettingsHandler(this._hpackEncoder, this._frameWriter, this._acknowledgedSettings, this._peerSettings);
  ...
  void _modifySettings(ActiveSettings base, List<Setting> changes, bool peerSettings) {
      ...
      switch (setting.identifier) {
        case Setting.SETTINGS_ENABLE_PUSH:
          if (setting.value == 0) {
            ...
            throw ProtocolException('The push setting can be only set to 0 or 1.');
          }
          break;
