...
class HPackContext {
  ...
  HPackContext({int maxSendingHeaderTableSize = 4096, int maxReceivingHeaderTableSize = 4096}) {
    ...
    try {
      ...
      while (offset < data.length) {
        ...
        if (isIndexedField) {
          ...
          headers.add(readHeaderFieldInternal(readInteger(4), neverIndexed: true));
...
class IndexTable {
  ...
  Header lookup(int index) {
    if (index <= 0) {
      throw HPackDecodingException('Invalid index (was: $index) for table lookup.');
    }
