...
/// window allows.
class StreamMessageQueueOut extends Object with TerminatableMixin, ClosableMixin {
  ...
  StreamMessageQueueOut(this.streamId, this.streamWindow, this.connectionMessageQueue) {
      ...
      if (message is HeadersMessage) {
        ...
        if (bytesAvailable > 0 || message.bytes.isEmpty) {
          ...
          if (messageBytes.length > bytesAvailable) {
            ...
            var messageB = DataMessage(message.streamId, partB, message.endStream);
...
class StreamMessageQueueIn extends Object with TerminatableMixin, ClosableMixin, CancellableMixin {
    ...
    ensureNotClosingSync(() {
      if (!wasTerminated) {
        if (message is PushPromiseMessage) {
          ...
          var transportStreamPush = TransportStreamPush(message.headers, message.pushedStream);
  ...
  void forceDispatchIncomingMessages() {
    while (_pendingMessages.isNotEmpty) {
      ...
      if (message is HeadersMessage) {
        _incomingMessagesC.add(HeadersStreamMessage(message.headers, endStream: message.endStream));
      } else if (message is DataMessage) {
        if (message.bytes.isNotEmpty) {
          _incomingMessagesC.add(DataStreamMessage(message.bytes, endStream: message.endStream));
        }
      } else {
