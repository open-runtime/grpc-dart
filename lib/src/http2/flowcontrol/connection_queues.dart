...
class ConnectionMessageQueueOut extends Object with TerminatableMixin, ClosableMixin {
  ...
  void _trySendMessages() {
    if (!wasTerminated) {
      ...
      if (_messages.isNotEmpty &&
          ...
          (!_connectionWindow.positiveWindow.wouldBuffer || _messages.first is! DataMessage)) {
        _trySendMessage();
  ...
  void _trySendMessage() {
    ...
    if (message is HeadersMessage) {
      ...
      _frameWriter.writeHeadersFrame(message.streamId, message.headers, endStream: message.endStream);
    } else if (message is PushPromiseMessage) {
      _messages.removeFirst();
      _frameWriter.writePushPromiseFrame(message.streamId, message.promisedStreamId, message.headers);
      ...
      if (_connectionWindow.peerWindowSize >= message.bytes.length) {
        ...
        _frameWriter.writeDataFrame(message.streamId, message.bytes, endStream: message.endStream);
      } else {
        ...
        var tail = viewOrSublist(message.bytes, len, message.bytes.length - len);

        _connectionWindow.decreaseWindow(head.length);
        _frameWriter.writeDataFrame(message.streamId, head, endStream: false);

        var tailMessage = DataMessage(message.streamId, tail, message.endStream);
      ...
      _messages.removeFirst();
      _frameWriter.writeGoawayFrame(message.lastStreamId, message.errorCode, message.debugData);
    } else {
...
//   * all streams have been closed
//   * the connection state is finishing
class ConnectionMessageQueueIn extends Object with TerminatableMixin, ClosableMixin {
  /// The handler which will be used for increasing the connection-level flow
  /// control window.
  ...
  ConnectionMessageQueueIn(this._windowUpdateHandler, this._catchProtocolErrors);

  @override
  void onTerminated(Object? error) {
    // Defensively clear any remaining stream queues. Under normal shutdown,
    // _streams.terminate() removes all entries first. But if _terminate()
    // fires via FrameWriter.doneFuture.whenComplete (transport error) while
    // a new stream is being registered, entries may still exist. Clearing
    // instead of asserting prevents a crash on this timing race.
  ...
  void processHeadersFrame(HeadersFrame frame) {
    var streamId = frame.header.streamId;
    var message = HeadersMessage(streamId, frame.decodedHeaders, frame.hasEndStreamFlag);
  ...
  /// stream.
  void processPushPromiseFrame(PushPromiseFrame frame, ClientTransportStream pushedStream) {
    var streamId = frame.header.streamId;
    var message = PushPromiseMessage(streamId, frame.decodedHeaders, frame.promisedStreamId, pushedStream, false);
  ...
  void _tryDispatch(int streamId, StreamMessageQueueIn mq, Queue<Message> pendingMessages) {
