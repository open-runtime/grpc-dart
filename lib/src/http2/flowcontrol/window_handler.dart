...
class IncomingWindowHandler {
  ...
  IncomingWindowHandler.stream(this._frameWriter, this._localWindow, this._streamId);

  IncomingWindowHandler.connection(this._frameWriter, this._localWindow) : _streamId = 0;
  ...
  void gotData(int numberOfBytes) {
    ...
    if (_localWindow.size < 0) {
      throw FlowControlException('Connection level flow control window became negative.');
    }
  }
