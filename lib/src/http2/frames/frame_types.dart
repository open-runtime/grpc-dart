...
class FrameHeader {
  ...
  Map toJson() => {'length': length, 'type': type, 'flags': flags, 'streamId': streamId};
}
...
class HeadersFrame extends Frame {
  ...
  HeadersFrame addBlockContinuation(ContinuationFrame frame) {
    ...
    var fh = FrameHeader(header.length + fragment.length, header.type, flags, header.streamId);

    var mergedHeaderBlockFragment = Uint8List(headerBlockFragment.length + fragment.length);

    mergedHeaderBlockFragment.setRange(0, headerBlockFragment.length, headerBlockFragment);

    mergedHeaderBlockFragment.setRange(headerBlockFragment.length, mergedHeaderBlockFragment.length, fragment);

    return HeadersFrame(fh, padLength, exclusiveDependency, streamDependency, weight, mergedHeaderBlockFragment);
  }
...
class PriorityFrame extends Frame {
  ...
  PriorityFrame(super.header, this.exclusiveDependency, this.streamDependency, this.weight);

  @override
  Map toJson() =>
      super.toJson()
        ..addAll({'exclusiveDependency': exclusiveDependency, 'streamDependency': streamDependency, 'weight': weight});
}
...
class SettingsFrame extends Frame {
  ...
  @override
  Map toJson() => super.toJson()..addAll({'settings': settings.map((s) => s.toJson()).toList()});
}
...
class PushPromiseFrame extends Frame {
  ...
  PushPromiseFrame(super.header, this.padLength, this.promisedStreamId, this.headerBlockFragment);
  ...
  PushPromiseFrame addBlockContinuation(ContinuationFrame frame) {
    ...
    return PushPromiseFrame(fh, padLength, promisedStreamId, mergedHeaderBlockFragment);
  }
...
class GoawayFrame extends Frame {
  ...
  Map toJson() =>
      super.toJson()
        ..addAll({'lastStreamId': lastStreamId, 'errorCode': errorCode, 'debugData (length)': debugData.length});
}
...
class WindowUpdateFrame extends Frame {
  ...
  @override
  Map toJson() => super.toJson()..addAll({'windowSizeIncrement': windowSizeIncrement});
}
...
class ContinuationFrame extends Frame {
  ...
  @override
  Map toJson() => super.toJson()..addAll({'headerBlockFragment (length)': headerBlockFragment.length});
}
