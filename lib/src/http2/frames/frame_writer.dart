...
class FrameWriter {
  ...
  FrameWriter(this._hpackEncoder, StreamSink<List<int>> outgoing, this._peerSettings)
  ...
  void writeDataFrame(int streamId, List<int> data, {bool endStream = false}) {
    while (data.length > _peerSettings.maxFrameSize) {
      ...
      data = viewOrSublist(data, _peerSettings.maxFrameSize, data.length - _peerSettings.maxFrameSize);
  ...
  void writeHeadersFrame(int streamId, List<Header> headers, {bool endStream = true}) {
    var fragment = _hpackEncoder.encode(headers);
    var maxSize = _peerSettings.maxFrameSize - HeadersFrame.MAX_CONSTANT_PAYLOAD;
  ...
  void _writeHeadersFrameNoFragment(int streamId, List<int> fragment, bool endHeaders, bool endStream) {
  ...
  void _writeContinuationFrame(int streamId, List<int> fragment, bool endHeaders) {
  ...
  void writePriorityFrame(int streamId, int streamDependency, int weight, {bool exclusive = false}) {
    var type = FrameType.PRIORITY;
    var flags = 0;

    var buffer = Uint8List(FRAME_HEADER_SIZE + PriorityFrame.FIXED_FRAME_LENGTH);
    var offset = 0;
  ...
  void writeRstStreamFrame(int streamId, int errorCode) {
    ...
    var buffer = Uint8List(FRAME_HEADER_SIZE + RstStreamFrame.FIXED_FRAME_LENGTH);
    var offset = 0;
  ...
  void writePushPromiseFrame(int streamId, int promisedStreamId, List<Header> headers) {
    var fragment = _hpackEncoder.encode(headers);
    var maxSize = _peerSettings.maxFrameSize - PushPromiseFrame.MAX_CONSTANT_PAYLOAD;

    if (fragment.length < maxSize) {
      _writePushPromiseFrameNoFragmentation(streamId, promisedStreamId, fragment, true);
    } else {
      var chunk = fragment.sublist(0, maxSize);
      fragment = fragment.sublist(maxSize);
      _writePushPromiseFrameNoFragmentation(streamId, promisedStreamId, chunk, false);
      while (fragment.length > _peerSettings.maxFrameSize) {
        var chunk = fragment.sublist(0, _peerSettings.maxFrameSize);
        fragment = fragment.sublist(_peerSettings.maxFrameSize);
        _writeContinuationFrame(streamId, chunk, false);
  ...
  void _writePushPromiseFrameNoFragmentation(int streamId, int promisedStreamId, List<int> fragment, bool endHeaders) {
  ...
  void writeWindowUpdate(int sizeIncrement, {int streamId = 0}) {
    ...
    var buffer = Uint8List(FRAME_HEADER_SIZE + WindowUpdateFrame.FIXED_FRAME_LENGTH);
    var offset = 0;
  ...
  void _setFrameHeader(List<int> bytes, int offset, int type, int flags, int streamId, int length) {
