...
class FrameReader {
  ...
  Stream<Frame> startDecoding() {
    ...
    Frame? tryReadFrame(FrameHeader header) {
      ...
      if (bufferedLength >= totalFrameLen) {
        ...
        if (firstChunkLen == totalFrameLen) {
          ...
          bufferedData[0] = viewOrSublist(bufferedData[0], totalFrameLen, firstChunkLen - totalFrameLen);
        }
    ...
    _framesController.onListen = () {
      ...
      subscription = _inputStream.listen(
            ...
            while (true) {
              ...
              if (header != null) {
                if (header!.length > _localSettings.maxFrameSize) {
                  terminateWithError(FrameSizeException('Incoming frame is too big.'));
                  return;
                }
        ...
        onDone: () {
          if (bufferedLength == 0) {
            ...
            terminateWithError(FrameSizeException('Incoming byte stream ended with incomplete frame'));
          }
        },
      );
      ...
      // NOTE: onCancel is deliberately NOT set. Cancelling the outer
      // subscription (from Connection._terminate) must NOT propagate to
      // the inner transport subscription. The transport stream is owned
      // by the Connection and closed separately via FrameWriter.close()
      // or socket.destroy(). Propagating cancel here would close the
      // transport prematurely, preventing graceful GOAWAY/finish().
    };
  ...
  void _mergeLists(List<List<int>> bufferedData, int numberOfBytes) {
    if (bufferedData[0].length < numberOfBytes) {
      ...
      while (accumulatedLength < numberOfBytes && numLists <= bufferedData.length) {
  ...
  Frame _readFrame(FrameHeader header, List<int> bytes, int frameOffset) {
    ...
    switch (header.type) {
      ...
      case FrameType.HEADERS:
        ...
        return HeadersFrame(header, padLength, exclusiveDependency, streamDependency, weight, headerBlockFragment);
      ...
      case FrameType.PRIORITY:
        ...
        return PriorityFrame(header, exclusiveDependency, streamDependency, weight);
      ...
      case FrameType.SETTINGS:
        ...
        if (frame.hasAckFlag) {
          _checkFrameLengthCondition(header.length == 0, message: 'Settings frame length must 0 for ACKs.');
        }
        return frame;
      ...
      case FrameType.PUSH_PROMISE:
        ...
        _checkFrameLengthCondition(headerBlockLen >= 0);
        var headerBlockFragment = viewOrSublist(bytes, offset, headerBlockLen);
        return PushPromiseFrame(header, padLength, promisedStreamId, headerBlockFragment);
      ...
      case FrameType.CONTINUATION:
        var headerBlockFragment = viewOrSublist(bytes, offset, frameEnd - offset);
        return ContinuationFrame(header, headerBlockFragment);

      default:
        // Unknown frames should be ignored according to spec.
        return UnknownFrame(header, viewOrSublist(bytes, offset, frameEnd - offset));
    }
  }

  /// Checks that [condition] is `true` and raises an [FrameSizeException]
  /// otherwise.
  void _checkFrameLengthCondition(bool condition, {String message = 'Frame not long enough.'}) {
    if (!condition) {
