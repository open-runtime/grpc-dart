...
class ConnectionState {
  ...
  bool get activeFinishing => state == Finishing && (finishingState & FinishingActive) != 0;

  bool get passiveFinishing => state == Finishing && (finishingState & FinishingPassive) != 0;
...
abstract class Connection {
  ...
  /// the peer.
  Future<void> get onInitialPeerSettingsReceived => _onInitialPeerSettingsReceived.future;
  ...
  /// peer.
  void _setupConnection(Stream<List<int>> incoming, StreamSink<List<int>> outgoing, Settings settingsObject) {
    // Setup frame reading.
    var incomingFrames = FrameReader(incoming, acknowledgedSettings).startDecoding();
    ...
    // Setup handlers.
    _settingsHandler = SettingsHandler(_hpackContext.encoder, _frameWriter, acknowledgedSettings, peerSettings);
    ...
    _settingsHandler.changeSettings(settings).catchError((Object error) {
      ...
      _terminate(ErrorCode.PROTOCOL_ERROR, message: 'Failed to set initial settings (error: $error).');
    });
  ...
  List<Setting> _decodeSettings(Settings settings) {
    ...
    if (concurrentStreamLimit != null) {
      settingsList.add(Setting(Setting.SETTINGS_MAX_CONCURRENT_STREAMS, concurrentStreamLimit));
    }
    ...
    if (streamWindowSize != null) {
      settingsList.add(Setting(Setting.SETTINGS_INITIAL_WINDOW_SIZE, streamWindowSize));
    }
  ...
  Future<void> ping() {
    return _pingHandler.ping().catchError((e, s) {
      return Future<void>.error(TransportException('The connection has been terminated.'));
  ...
  Future<void> finish() {
    ...
    return _streams.done.whenComplete(() => Future.wait([_frameWriter.close(), _frameReaderSubscription.cancel()]));
  }
  ...
  void _activeStateHandler(bool isActive) => onActiveStateChanged?.call(isActive);
  ...
  void _handleFrameImpl(Frame? frame) {
    ...
    if (_state.isInitialized) {
      if (frame is! SettingsFrame) {
        _terminate(ErrorCode.PROTOCOL_ERROR, message: 'Expected to first receive a settings frame.');
        return;
      }
    ...
    if (frame is HeadersFrame) {
      frame.decodedHeaders = _hpackContext.decoder.decode(frame.headerBlockFragment);
    ...
    if (frame.header.streamId == 0) {
      if (frame is SettingsFrame) {
        ...
        throw ProtocolException('Cannot handle frame type ${frame.runtimeType} with stream-id 0.');
      }
    } else {
  ...
  Future _terminate(int errorCode, {bool causedByTransportError = false, String? message}) {
    ...
    if (_state.state != ConnectionState.Terminated) {
      ...
      if (!causedByTransportError) {
        _outgoingQueue.enqueueMessage(
          GoawayMessage(_streams.highestPeerInitiatedStream, errorCode, message != null ? utf8.encode(message) : []),
        );
      }
      ...
      var exception = TransportConnectionException(errorCode, 'Connection is being forcefully terminated.');
      ...
      // Close the ping and frame-received controllers. Without this, any
      // active subscription (e.g., ServerKeepAlive) on these streams keeps
      // the Dart VM event loop alive indefinitely — the root cause of
      // 44-minute Windows CI hangs.
      _pingReceived.close();
      _frameReceived.close();

      return Future.wait([cancelFuture, closeFuture]).catchError((_) => const <void>[]);
    }
...
class ClientConnection extends Connection implements ClientTransportConnection {
  ClientConnection._(super.incoming, super.outgoing, super.settings) : super(isClientConnection: true);

  factory ClientConnection(Stream<List<int>> incoming, StreamSink<List<int>> outgoing, ClientSettings clientSettings) {
    outgoing.add(CONNECTION_PREFACE);
    return ClientConnection._(incoming, outgoing, clientSettings);
  }

  @override
  bool get isOpen => !_state.isFinishing && !_state.isTerminated && _streams.canOpenStream;
...
class ServerConnection extends Connection implements ServerTransportConnection {
  ServerConnection._(super.incoming, super.outgoing, super.settings) : super(isClientConnection: false);

  factory ServerConnection(Stream<List<int>> incoming, StreamSink<List<int>> outgoing, ServerSettings serverSettings) {
    var frameBytes = readConnectionPreface(incoming);
    return ServerConnection._(frameBytes, outgoing, serverSettings);
  }

  @override
  Stream<ServerTransportStream> get incomingStreams => _streams.incomingStreams.cast<ServerTransportStream>();
