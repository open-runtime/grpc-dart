...
class BufferedBytesWriter {
  ...
  BufferedBytesWriter(StreamSink<List<int>> outgoing) : _bufferedSink = BufferedSink(outgoing);
  ...
  /// If the writer has been closed, the data is silently dropped.
  void add(List<int> data) {
    if (_closed) return;
    if (_builder.length > 0) {
      throw StateError('Cannot trigger an asynchronous write while there is buffered data.');
    }
