...
class MockHttpRequest extends Mock implements IXMLHttpRequest {
  ...
  @override
  String get responseText => super.noSuchMethod(Invocation.getter(#responseText), returnValue: '');

  @override
  int get readyState => super.noSuchMethod(Invocation.getter(#readyState), returnValue: -1);

  @override
  Map<String, String> get responseHeaders =>
      super.noSuchMethod(Invocation.getter(#responseHeaders), returnValue: <String, String>{});
}

class MockXhrClientConnection extends XhrClientConnection {
  MockXhrClientConnection({int? code}) : _statusCode = code ?? 200, super(Uri.parse('test:0'));
...
void main() {
  test('Make request sends correct headers', () async {
    final metadata = <String, String>{'parameter_1': 'value_1', 'parameter_2': 'value_2'};

    final connection = MockXhrClientConnection();

    connection.makeRequest('path', Duration(seconds: 10), metadata, (error, _) => fail(error.toString()));

    verify(connection.latestRequest.setRequestHeader('Content-Type', 'application/grpc-web+proto'));
  ...
  test('Make request sends correct headers and path if bypassCorsPreflight=true', () async {
  ...
  test('Make request sends correct headers if call options already have '
    ...
    final metadata = {'header_1': 'value_1', 'header_2': 'value_2', 'Content-Type': 'application/json+protobuf'};
    final connection = MockXhrClientConnection();

    connection.makeRequest('/path', Duration(seconds: 10), metadata, (error, _) => fail(error.toString()));

    expect(metadata, {'header_1': 'value_1', 'header_2': 'value_2', 'Content-Type': 'application/json+protobuf'});
  });
  ...
  test('Content-Type header case insensitivity', () async {
    ...
    connection.makeRequest('/path', Duration(seconds: 10), metadata, (error, _) => fail(error.toString()));
    expect(metadata, {'header_1': 'value_1', 'CONTENT-TYPE': 'application/json+protobuf'});

    final lowerMetadata = {'header_1': 'value_1', 'content-type': 'application/json+protobuf'};
    connection.makeRequest('/path', Duration(seconds: 10), lowerMetadata, (error, _) => fail(error.toString()));
    expect(lowerMetadata, {'header_1': 'value_1', 'content-type': 'application/json+protobuf'});
  });

  test('Make request sends correct headers path if only withCredentials=true', () async {
    final metadata = {'header_1': 'value_1', 'header_2': 'value_2'};
    final connection = MockXhrClientConnection();
  ...
  test('Stream handles headers properly', () async {
    ...
    final stream = transport.makeRequest('test_path', Duration(seconds: 10), {}, (error, _) => fail(error.toString()));

    when(transport.latestRequest.responseHeaders).thenReturn(responseHeaders);
    when(transport.latestRequest.responseText).thenReturn(String.fromCharCodes(frame(<int>[])));
    ...
    transport.latestRequest.readyStateChangeController.add(readyStateChangeEvent);
    transport.latestRequest.readyStateChangeController.add(readyStateChangeEvent);
  ...
  test('Stream handles trailers properly', () async {
    final requestHeaders = {'parameter_1': 'value_1', 'content-type': 'application/grpc+proto'};
    final responseTrailers = <String, String>{'trailer_1': 'value_1', 'trailer_2': 'value_2'};
    ...
    final encodedTrailers = frame(responseTrailers.entries.map((e) => '${e.key}:${e.value}').join('\r\n').codeUnits);
    ...
    when(connection.latestRequest.readyState).thenReturnInOrder([XMLHttpRequest.HEADERS_RECEIVED, XMLHttpRequest.DONE]);
    connection.latestRequest.readyStateChangeController.add(readyStateChangeEvent);
    connection.latestRequest.progressController.add(progressEvent);
    connection.latestRequest.readyStateChangeController.add(readyStateChangeEvent);

    // Should be two metadata messages: headers and trailers.
    final messages = await stream.incomingMessages.whereType<GrpcMetadata>().toList();
  ...
  test('Stream handles empty trailers properly', () async {
    ...
    final stream = connection.makeRequest('test_path', Duration(seconds: 10), {}, (error, _) => fail(error.toString()));
  ...
  test('Stream deserializes data properly', () async {
    ...
    when(connection.latestRequest.responseHeaders).thenReturn(requestHeaders);
    when(connection.latestRequest.responseText).thenReturn(String.fromCharCodes(frame(data)));
  ...
  test('GrpcError with error details in response', () async {
    ...
    when(connection.latestRequest.responseHeaders).thenReturn({'content-type': 'application/grpc+proto'});
  ...
  test('Stream receives multiple messages', () async {
    ...
    final data = <List<int>>[List<int>.filled(10, 224), List<int>.filled(5, 124)];
    final encodedStrings = data.map((d) => String.fromCharCodes(frame(d))).toList();

    when(connection.latestRequest.responseHeaders).thenReturn(metadata);
    when(connection.latestRequest.readyState).thenReturn(XMLHttpRequest.HEADERS_RECEIVED);
    ...
    final readyStates = [XMLHttpRequest.HEADERS_RECEIVED, XMLHttpRequest.DONE];
    when(connection.latestRequest.readyState).thenAnswer((_) => readyStates.removeAt(0));
