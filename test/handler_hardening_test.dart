...
class ErrorCapturingHarness {
  ...
  void sendRequestHeader(String path, {Duration? timeout, Map<String, String>? metadata}) {
...
class _MultiBlockingBidiService extends Service {
  ...
  _MultiBlockingBidiService({required this.handlersEntered, required this.handlersExited}) {
    $addMethod(
      ServiceMethod<int, int>(
        'BidiStream',
        _bidiStream,
...
class _CountingDecodeBidiService extends Service {
  ...
  _CountingDecodeBidiService({required this.onDeserialize, required this.onFirstRequestSeen}) {
    $addMethod(
      ServiceMethod<int, int>('BidiStream', _bidiStream, true, true, (List<int> value) {
        onDeserialize();
        return value[0];
      }, (int value) => [value]),
    );
...
class _ThrowingVoidSink implements Sink<void> {
  final void Function() onAddAttempt;

  _ThrowingVoidSink({required this.onAddAttempt});

  @override
  void add(void data) {
    onAddAttempt();
    throw StateError('forced keepalive notifier failure');
  }
...
class _ThrowingAddErrorController<T> implements StreamController<T> {
  ...
  @override
  set onCancel(FutureOr<void> Function()? handler) => _delegate.onCancel = handler;
...
class _ThrowingAddErrorMethod extends ServiceMethod<int, int> {
  ...
  _ThrowingAddErrorMethod(this.onAddErrorAttempt, Stream<int> Function(ServiceCall, Stream<int>) handler)
    : super('BidiStream', handler, true, true, (List<int> value) => value[0], (int value) => [value]);
...
class _ThrowingAddErrorResponseErrorMethod extends ServiceMethod<int, int> {
  ...
  _ThrowingAddErrorResponseErrorMethod(this.onAddErrorAttempt, Stream<int> Function(ServiceCall, Stream<int>) handler)
    : super('ResponseError', handler, true, true, (List<int> value) => value[0], (int value) => throw 'Failed');
...
class _ThrowingAddErrorUnaryMethod extends ServiceMethod<int, int> {
  ...
  _ThrowingAddErrorUnaryMethod(this.onAddErrorAttempt, Future<int> Function(ServiceCall, Future<int>) handler)
    : super('NoRequestUnary', handler, false, false, (List<int> value) => value[0], (int value) => [value]);
...
class _OnDoneExpectedCatchService extends Service {
  ...
  _OnDoneExpectedCatchService({required this.onAddErrorAttempt, required this.onExited}) {
...
class _OnResponseCatchBidiService extends Service {
  ...
  _OnResponseCatchBidiService({
    ...
    $addMethod(_ThrowingAddErrorResponseErrorMethod(onAddErrorAttempt, _bidiStream));
  }
...
void main() {
  ...
  group('sendTrailers double-call guard', () {
    ...
    test('handler error produces exactly one trailer (guard prevents duplicates)', () async {
    ...
    test('_trailersSent guard blocks second sendTrailers from concurrent path', () async {
      ...
      harness.service.serverStreamingHandler = methodHandler;
      harness.sendRequestHeader('/Test/ServerStreaming', timeout: const Duration(microseconds: 1));
      harness.sendData(1);
      harness.toServer.close();
    ...
    test('yield-then-throw triggers both response paths without crash', () async {
      Stream<int> methodHandler(ServiceCall call, Future<int> request) async* {
        await request;
        yield 1;
        yield 2;
      ...
      expect(trailerCount, equals(1), reason: 'Expected exactly 1 trailer with grpc-status, got $trailerCount');
    });

    test('sendTrailers handles closed outgoing sink without crashing', () async {
      final errorHarness = ErrorCapturingHarness()..setUp();
      addTearDown(() => errorHarness.tearDown());

      final handlerReached = Completer<void>();
      errorHarness.service.unaryHandler = (ServiceCall call, Future<int> request) async {
      ...
      await handlerReached.future.timeout(const Duration(seconds: 5), onTimeout: () => fail('Handler never started'));
  ...
  group('_onTimedOut TOCTOU safety', () {
    ...
    test('timeout with isCanceled TOCTOU sends DEADLINE_EXCEEDED', () async {
      ...
      expect(trailerCount, equals(1), reason: 'Exactly 1 trailer expected, got $trailerCount');
      // The handler should NOT have completed all 100 yields.
      expect(yieldsBeforeCancel, lessThan(100), reason: 'Timeout should have stopped the handler early');
    });
    ...
    test('timeout timer firing after handler completion is harmless', () async {
      ...
      harness.sendRequestHeader('/Test/Unary', timeout: const Duration(milliseconds: 50));
      harness.sendData(42);
      harness.toServer.close();
      ...
      expect(sawOkStatus, isTrue, reason: 'Handler should have completed with OK status');
      expect(
        trailerCount,
        equals(1),
        reason:
  ...
  group('_onDataActive guards', () {
    ...
    test('data arriving after stream closure is handled gracefully', () async {
      ...
      expect(
        harness.capturedErrors.any((e) => e.message?.contains('Immediate failure') ?? false),
        isTrue,
  ...
  // ---------------------------------------------------------------------------
  // Keepalive notifier hardening
  // ---------------------------------------------------------------------------
  group('keepalive notifier hardening', () {
    test('notifier add failure does not break valid unary processing', () async {
      final toServer = StreamController<StreamMessage>();
      final fromServer = StreamController<StreamMessage>();
      final service = TestService();
      var notifierAddAttempts = 0;

      service.unaryHandler = (call, request) async => await request;

      final server = Server.create(services: [service], errorHandler: (_, _) {});
      final stream = TestServerStream(toServer.stream, fromServer.sink);
      server.serveStream_(
        stream: stream,
        onDataReceived: _ThrowingVoidSink(onAddAttempt: () => notifierAddAttempts++),
      );
      ...
      fromServer.stream.listen(
        ...
        onError: (_) {
          if (!wireSettled.isCompleted) wireSettled.complete();
        },
        onDone: () {
          if (!wireSettled.isCompleted) wireSettled.complete();
        },
      ...
      toServer.add(HeadersStreamMessage(headers));
      toServer.add(DataStreamMessage(frame(mockEncode(7))));
      await toServer.close();

      await wireSettled.future.timeout(
        const Duration(seconds: 3),
        onTimeout: () => fail('Wire did not settle for notifier throw unary flow'),
      );
      ...
      expect(
        ...
        equals(2),
        reason:
            'Notifier should be attempted in both _onDataIdle (header) '
            'and _onDataActive (request data)',
      );
      expect(
        grpcStatus,
        equals('${StatusCode.ok}'),
        reason:
            'Notifier failure must not corrupt successful request '
            'processing or trailers',
      );
    });

    test('_onDataIdle notifier failure preserves header-frame protocol error', () async {
      final toServer = StreamController<StreamMessage>();
      final fromServer = StreamController<StreamMessage>();
      final service = TestService();
      var notifierAddAttempts = 0;
      final capturedErrors = <GrpcError>[];
      ...
      // Send data before headers to trigger the protocol error path in
      // _onDataIdle while the notifier throws.
      toServer.add(DataStreamMessage(frame(mockEncode(1))));
      await toServer.close();

      await wireSettled.future.timeout(
        const Duration(seconds: 3),
        onTimeout: () => fail('Wire did not settle for notifier throw + invalid header frame'),
      );

      expect(notifierAddAttempts, equals(1));
      expect(
        grpcStatus,
        equals('${StatusCode.unimplemented}'),
        reason:
            'Notifier failure must not mask the Expected header frame '
            'protocol error with INTERNAL',
      );
      expect(
        grpcMessage,
        contains('Expected header frame'),
        reason:
            'Protocol-level message should remain visible when notifier '
            'updates fail',
      );
      expect(capturedErrors, hasLength(1));
      expect(capturedErrors.single.code, equals(StatusCode.unimplemented));
    });
  });
  ...
  group('_onResponse safe error handling', () {
    ...
    test('serialization error during streaming does not crash server', () async {
      Stream<int> methodHandler(ServiceCall call, Stream<int> request) async* {
        await for (final value in request) {
          yield value;
        }
      ...
      expect(grpcStatusCode, isNotNull, reason: 'Server should have sent grpc-status trailer');
  ...
  group('cancel() unblocks await-for handlers', () {
    test('bidi handler blocked in await-for completes on cancel()', () async {
      ...
      await handlerUnblocked.future.timeout(
        const Duration(seconds: 2),
        onTimeout: () => fail('Handler stayed blocked — cancel() did not close _requests'),
      );
      expect(sawCancelledError, isTrue, reason: 'Expected handler to be unblocked by GrpcError.cancelled');
    });
  ...
  group('_onDoneExpected safe error handling', () {
      ...
      harness.fromServer.stream.listen(
        (message) {
          if (message is HeadersStreamMessage) {
            ...
            if (grpcMessage != null && grpcMessage.contains('No request received')) {
    ...
    test('_onDoneExpected path tolerates request-stream addError failure, '
      ...
      await wireDone.future.timeout(
        const Duration(seconds: 3),
        onTimeout: () => fail('Wire did not settle after _onDoneExpected no-request path'),
      );
      ...
      await handlerExited.future.timeout(
        const Duration(seconds: 2),
        onTimeout: () => fail('Handler did not exit after _onDoneExpected with addError throw'),
      );
    });
  });
  ...
  group('Server.shutdown() end-to-end production path', () {
    testTcpAndUds('Server.shutdown() unblocks bidi handler stuck in await-for', (address) async {
      final handlerEntered = Completer<void>();
      final handlerExited = Completer<void>();

      final service = _BlockingBidiService(onEntered: handlerEntered, onExited: handlerExited);
      ...
      await server.shutdown().timeout(
        const Duration(seconds: 5),
        onTimeout: () => fail('server.shutdown() hung — cancel() did not unblock handler'),
      );
      ...
      await streamDone.future.timeout(
        const Duration(seconds: 2),
        onTimeout: () => fail('Bidi response stream did not settle after shutdown'),
      );
      await channel.shutdown();
    });

    testTcpAndUds('Server.shutdown() with 10 concurrent blocked bidi handlers', (address) async {
      ...
      final service = _MultiBlockingBidiService(handlersEntered: handlersEntered, handlersExited: handlersExited);
      ...
      await Future.wait(
        handlersEntered.map(
          (c) => c.future.timeout(const Duration(seconds: 5), onTimeout: () => fail('Not all handlers entered')),
        ),
      );
      ...
      await server.shutdown().timeout(
        ...
        onTimeout: () => fail('shutdown() hung with 10 blocked bidi handlers'),
      );
  ...
  group('Handler state machine adversarial races', () {
    ...
    test('data arriving after cancel() is silently discarded', () async {
      ...
      expect(handler.isCanceled, isTrue);
      expect(sawCancelledError, isTrue, reason: 'Handler should have received GrpcError.cancelled');
    ...
    test('cancel() stops request deserialization after cancellation', () async {
      ...
      expect(
        ...
        reason: 'No additional request deserialization should occur after cancel',
      );
    });
    ...
    test('timeout fires while handler is mid-yield', () async {
      ...
      expect(trailerCount, equals(1), reason: 'Expected exactly 1 trailer, got $trailerCount');
      expect(sawDeadlineExceeded, isTrue);
    });
    ...
    test('_onError + _onDone + cancel() triple-race is safe', () async {
      ...
      expect(toServer.isClosed, isFalse, reason: 'Input stream closed before triple-race injection');
    ...
    test('_onError catch path tolerates request-stream addError failure', () async {
      final toServer = StreamController<StreamMessage>();
      final fromServer = StreamController<StreamMessage>();
      final firstRequestSeen = Completer<void>();
      final handlerExited = Completer<void>();
      ...
      await handlerExited.future.timeout(
        const Duration(seconds: 2),
        onTimeout: () => fail('Handler did not exit after _onError with addError throw'),
      );
    });

    test('cancel() path tolerates request-stream addError failure, handler exits', () async {
      final toServer = StreamController<StreamMessage>();
      final fromServer = StreamController<StreamMessage>();
      final firstRequestSeen = Completer<void>();
      final handlerExited = Completer<void>();
      ...
      await handlerExited.future.timeout(
        const Duration(seconds: 2),
        onTimeout: () => fail('Handler did not exit after cancel() with addError throw'),
      );
    });

    test('_onResponse path tolerates request-stream addError failure, handler exits', () async {
      final toServer = StreamController<StreamMessage>();
      final fromServer = StreamController<StreamMessage>();
      final firstRequestSeen = Completer<void>();
      final handlerExited = Completer<void>();
      ...
      await wireDone.future.timeout(
        const Duration(seconds: 3),
        onTimeout: () => fail('Wire did not settle after _onResponse serialization error'),
      );
      ...
      await handlerExited.future.timeout(
        const Duration(seconds: 2),
        onTimeout: () => fail('Handler did not exit after _onResponse with addError throw'),
      );
    });

    test('8 sequential handlers exercise all _requests-closing paths', () async {
      ...
      expect(path1Trailer, contains('grpc-status'), reason: 'Path 1 (normal unary) must send a trailer');
      expect(path1Trailer['grpc-status'], '0', reason: 'Path 1 must succeed with OK');
      ...
      expect(path2Trailer, contains('grpc-status'), reason: 'Path 2 (no-data unary) must send a trailer');
      ...
      expect(path4Result.handler.isCanceled, isTrue, reason: 'Path 4 handler must be canceled');
      ...
      expect(path5Trailer, contains('grpc-status'), reason: 'Path 5 (timeout) must send a trailer');
      expect(
      ...
      expect(path6Trailer, contains('grpc-status'), reason: 'Path 6 (deserialization error) must send a trailer');
      expect(
      ...
      expect(path7Trailer, contains('grpc-status'), reason: 'Path 7 (serialization error) must send a trailer');
      expect(
      ...
      expect(path8Trailer, contains('grpc-status'), reason: 'Path 8 (non-GrpcError throw) must send a trailer');
...
Future<Map<String, String>> _runHarnessHandler({
  ...
  await done.future.timeout(
    const Duration(seconds: 5),
    onTimeout: () => fail('_runHarnessHandler hung for $method — handler did not complete'),
  );
  if (!fromServer.isClosed) await fromServer.close();
  return lastTrailer;
}
...
Future<_CancelHandlerResult> _runCancelHandler() async {
  ...
  await handlerExited.future.timeout(const Duration(seconds: 3), onTimeout: () => fail('Cancel handler did not exit'));
