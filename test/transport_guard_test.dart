...
void main() {
  ...
  group('Transport sink guard under server shutdown', () {
        ...
        () async {
          try {
            ...
            final results = await Future.wait(
              futures,
            ).timeout(const Duration(seconds: 15), onTimeout: () => fail('Timed out waiting for streams'));

            expect(results.length, equals(10), reason: 'All 10 bidi streams must settle');
    ...
    testTcpAndUds('server shutdown during large payload transfer', (address) async {
      ...
      runZonedGuarded(
        () async {
          try {
            ...
            final futures = <Future<Object?>>[];
            for (var i = 0; i < 5; i++) {
              futures.add(settleRpc(client.echoBytes(payload).then<Object?>((v) => v)));
            }
            ...
            final results = await Future.wait(
              futures,
            ).timeout(const Duration(seconds: 15), onTimeout: () => fail('Timed out waiting for RPCs'));

            expect(results.length, equals(5), reason: 'All 5 echoBytes RPCs must settle');
            for (final r in results) {
              expectHardcoreRpcSettlement(
                r,
                reason:
        ...
        () async {
          try {
            ...
            for (var i = 0; i < 10; i++) {
              ...
              futures.add(settleRpc(client.bidiStreamBytes(ctrl.stream).toList().then<Object?>((v) => v)));
            }
            ...
            final feedResults = await Future.wait(
              feedFutures.map((future) => future.then<Object?>((_) => null, onError: (Object e) => e)),
            );
            ...
            expect(results.length, equals(10), reason: 'All 10 bidiStreamBytes streams must settle');
            for (final r in results) {
              expectHardcoreRpcSettlement(
                r,
                reason:
        ...
        () async {
          try {
            ...
            await Future.wait([
              server.shutdown(),
              channel.terminate(),
            ]).timeout(const Duration(seconds: 15), onTimeout: () => fail('Timed out during dual shutdown'));

            final results = await Future.wait(
              futures,
            ).timeout(const Duration(seconds: 15), onTimeout: () => fail('Timed out waiting for streams'));

            expect(results.length, equals(20), reason: 'All 20 bidi RPCs must settle without crashes');
            for (final r in results) {
              expectHardcoreRpcSettlement(r, reason: 'Each RPC must settle as success or known error type');
            }
      ...
      runZonedGuarded(
        () async {
          try {
            ...
            final futures = <Future<Object?>>[];
            for (var i = 0; i < 10; i++) {
              futures.add(settleRpc(client.serverStreamBytes(request).toList().then<Object?>((v) => v)));
            }
      ...
      runZonedGuarded(
        () async {
          try {
            ...
            final futures = <Future<Object?>>[];
            for (var i = 0; i < 10; i++) {
              futures.add(settleRpc(client.echoBytes(Uint8List(32768)).then<Object?>((v) => v)));
            }
  ...
  group('Error propagation through guarded sinks', () {
    testTcpAndUds('RPC errors do not crash when sink is closing', (address) async {
      final http2InternalErrors = <Object>[];
      final done = Completer<void>();
      runZonedGuarded(
        () async {
      ...
      runZonedGuarded(
        () async {
          try {
            ...
            final results = await Future.wait(
              futures,
            ).timeout(const Duration(seconds: 30), onTimeout: () => fail('Timed out waiting for streams'));

            expect(results.length, equals(50), reason: 'All 50 bidi streams must settle');
            for (final r in results) {
              expectHardcoreRpcSettlement(
                r,
                reason:
        ...
        () async {
          try {
            for (var cycle = 0; cycle < 10; cycle++) {
              ...
              final results = await Future.wait(
                futures,
              ).timeout(const Duration(seconds: 10), onTimeout: () => fail('Timed out on cycle $cycle'));

              expect(results.length, equals(5), reason: 'Cycle $cycle: all 5 streams must settle');
              for (final r in results) {
                expectHardcoreRpcSettlement(
                  r,
                  reason:
  ...
  group('Named pipe transport guard', () {
    testNamedPipe('server shutdown during active bidi data flow does not crash', (pipeName) async {
      ...
      final controllers = List.generate(streamCount, (_) => StreamController<int>());
      final futures = <Future<Object?>>[];
      ...
      for (var i = 0; i < results.length; i++) {
        expectHardcoreRpcSettlement(results[i], reason: 'Named-pipe stream $i settled with invalid result type');
      }
