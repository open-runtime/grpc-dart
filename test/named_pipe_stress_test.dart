...
// =============================================================================
// Test Service Definitions (mirrors transport_test.dart)
// =============================================================================
...
void main() {
  ...
  group('Named Pipe Lifecycle', () {
    ...
    testNamedPipe(
      'server start -> shutdown with no clients',
      (pipeName) async {
        final server = NamedPipeServer.create(services: [EchoService()]);
        await server.serve(pipeName: pipeName);
        expect(server.isRunning, isTrue);
    ...
    testNamedPipe(
      'server start -> shutdown -> restart with same pipe name',
      (pipeName) async {
    ...
    testNamedPipe(
      'multiple sequential server start/shutdown cycles',
      (pipeName) async {
    ...
    testNamedPipe(
      'server shutdown during active server-streaming RPC',
      (pipeName) async {
        final server = NamedPipeServer.create(services: [EchoService()]);
        await server.serve(pipeName: pipeName);
  ...
  group('Named Pipe Concurrency & Stress', () {
    ...
    testNamedPipe(
      '50 concurrent unary RPCs on same channel',
      (pipeName) async {
        final server = NamedPipeServer.create(services: [EchoService()]);
        await server.serve(pipeName: pipeName);
    ...
    testNamedPipe(
      '10 sequential channel connect/disconnect cycles',
      (pipeName) async {
        final server = NamedPipeServer.create(services: [EchoService()]);
        await server.serve(pipeName: pipeName);
    ...
    testNamedPipe(
      'multiple clients connecting to same server simultaneously',
      (pipeName) async {
        final server = NamedPipeServer.create(services: [EchoService()]);
        await server.serve(pipeName: pipeName);
    ...
    testNamedPipe(
      'rapid fire 100 sequential echo RPCs',
      (pipeName) async {
        final server = NamedPipeServer.create(services: [EchoService()]);
        await server.serve(pipeName: pipeName);
  ...
  group('Named Pipe Error Handling', () {
    ...
    testNamedPipe(
      'client connect to non-existent pipe throws error',
      (pipeName) async {
    ...
    testNamedPipe(
      'server with invalid pipe name rejects serve()',
      (pipeName) async {
    ...
    testNamedPipe(
      'client connect after server shutdown fails',
      (pipeName) async {
        final server = NamedPipeServer.create(services: [EchoService()]);
        await server.serve(pipeName: pipeName);
