...
import 'package:grpc/src/http2/frames/frame_defragmenter.dart';
...
void main() {
  group('frames', () {
    group('frame-defragmenter', () {
      ...
      HeadersFrame headersFrame(List<int> data, {bool fragmented = false, int streamId = 1}) {
        var flags = fragmented ? 0 : HeadersFrame.FLAG_END_HEADERS;
        var header = FrameHeader(data.length, FrameType.HEADERS, flags, streamId);
        return HeadersFrame(header, 0, false, null, null, data);
      }

      PushPromiseFrame pushPromiseFrame(List<int> data, {bool fragmented = false, int streamId = 1}) {
        var flags = fragmented ? 0 : HeadersFrame.FLAG_END_HEADERS;
        var header = FrameHeader(data.length, FrameType.PUSH_PROMISE, flags, streamId);
        return PushPromiseFrame(header, 0, 44, data);
      }

      ContinuationFrame continuationFrame(List<int> data, {bool fragmented = false, int streamId = 1}) {
        var flags = fragmented ? 0 : ContinuationFrame.FLAG_END_HEADERS;
        var header = FrameHeader(data.length, FrameType.CONTINUATION, flags, streamId);
      ...
      test('fragmented-headers-frame--wrong-id', () {
        ...
        expect(defrag.tryDefragmentFrame(f1), isNull);
        expect(() => defrag.tryDefragmentFrame(f2), throwsA(isProtocolException));
      });
