  ...
  group('hpack', () {
    group('hpack-spec-decoder', () {
      ...
      test('C.4 request with huffman encoding', () {
        ...
        // Second request
        headers = context.decoder.decode([0x82, 0x86, 0x84, 0xbe, 0x58, 0x86, 0xa8, 0xeb, 0x10, 0x64, 0x9c, 0xbf]);
        expect(headers, hasLength(5));
        expect(headers[0], isHeader(':method', 'GET'));
        expect(headers[1], isHeader(':scheme', 'http'));
        expect(headers[2], isHeader(':path', '/'));
      ...
      test('C.5 response without huffman encoding', () {
        ...
        // Second response
        headers = context.decoder.decode([0x48, 0x03, 0x33, 0x30, 0x37, 0xc1, 0xc0, 0xbf]);
        expect(headers, hasLength(4));
        ...
        expect(headers[5], isHeader('set-cookie', 'foo=ASDJKHQKBZXOQWEOPIUAXQWEOIU; max-age=3600; version=1'));
      });
      ...
      test('C.6 response with huffman encoding', () {
        ...
        // Second response
        headers = context.decoder.decode([0x48, 0x83, 0x64, 0x0e, 0xff, 0xc1, 0xc0, 0xbf]);
        expect(headers, hasLength(4));
        expect(headers[0], isHeader(':status', '307'));
        expect(headers[1], isHeader('cache-control', 'private'));
        expect(headers[2], isHeader('date', 'Mon, 21 Oct 2013 20:13:21 GMT'));
    ...
    group('negative-decoder-tests', () {
      test('invalid-integer-encoding', () {
        var context = HPackContext();
        expect(() => context.decoder.decode([1 << 6, 0xff]), throwsA(isHPackDecodingException));
      });

      test('index-out-of-table-size', () {
        var context = HPackContext();
        expect(() => context.decoder.decode([0x7f]), throwsA(isHPackDecodingException));
      });

      test('invalid-update-dynamic-table-size', () {
        var context = HPackContext();
        expect(() => context.decoder.decode([0x3f]), throwsA(isHPackDecodingException));
      });
      ...
      test('update-dynamic-table-size-too-high', () {
        ...
        expect(() => context.decoder.decode(bytes), throwsA(isHPackDecodingException));
      });
    });
    ...
    group('custom decoder tests', () {
      ...
      test('update-dynamic-table-size-too-high', () {
        ...
        expect(context.decoder.decode(TestHelper.newInteger(0x20, 5, 4096)), <void>[]);
      });
      ...
      test('dynamic table entry', () {
        ...
        // entry.
        headers = context.decoder.decode(TestHelper.setDynamicTableSize(4096 - 1));
        ...
        expect(() => context.decoder.decode(TestHelper.dynamicTableLookup(2)), throwsA(isHPackDecodingException));
      });
    });
    ...
    group('encoder-tests', () {
      test('simple-encoding', () {
        ...
        expect(context.encoder.encode(headers), [0x00, 0x03, 0x6b, 0x65, 0x79, 0x05, 0x76, 0x61, 0x6c, 0x75, 0x65]);
      });
      ...
      test('simple-encoding-long-value', () {
        ...
        expect(context.decoder.decode(context.encoder.encode(headers)).first, equalsHeader(headers.first));
...
class _HeaderMatcher extends Matcher {
  ...
  bool matches(Object? item, Map matchState) {
    return item is Header && _compareLists(item.name, header.name) && _compareLists(item.value, header.value);
  }
...
Matcher isHeader(String name, String value) => _HeaderMatcher(Header.ascii(name, value));
