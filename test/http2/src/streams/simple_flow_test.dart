        ...
        return expectAsync1((StreamMessage msg) {
          expect(
            ...
            isA<HeadersStreamMessage>()
                .having((m) => m.headers.first.name, 'headers.first.name', expectedHeaders.first.name)
                .having((m) => m.headers.first.value, 'headers.first.value', expectedHeaders.first.value),
          );
        });
      }
      ...
      void Function(StreamMessage) messageTestFun(String type) {
        ...
        return (StreamMessage msg) {
          if (expectHeader) {
            ...
            expect(bytes, allBytes.sublist(numBytesReceived, numBytesReceived + bytes.length));
      ...
      streamTest('single-header-request--empty-response', (
        ...
        server.incomingStreams.listen(
          expectAsync1((TransportStream sStream) async {
            sStream.incomingMessages.listen(messageTestFun('server'), onDone: expectAsync0(() {}));
        ...
        sendData(cStream);
        cStream.incomingMessages.listen(headersTestFun('client'), onDone: expectAsync0(() {}));
      });
    });
  });
}
