...
import 'package:grpc/src/http2/connection_preface.dart';
  ...
  group('client-tests', () {
    group('normal', () {
      clientTest('gracefull-shutdown-for-unused-connection', (
        ...
        Future serverFun() async {
          ...
          expect(await nextFrame(), isA<GoawayFrame>().having((f) => f.errorCode, 'errorCode', ErrorCode.NO_ERROR));
    ...
    group('connection-operational', () {
      clientTest('on-connection-operational-fires', (
        ...
        Future clientFun() async {
          await settingsDone.future;
          await client.onInitialPeerSettingsReceived.timeout(const Duration(milliseconds: 20)); // Should complete
      ...
      clientTest('on-connection-operational-does-not-fire', (
        ...
        Future clientFun() async {
          ...
          expect(
            client.onInitialPeerSettingsReceived.timeout(const Duration(seconds: 1)),
    ...
    group('server-errors', () {
      ...
      clientTest('data-frame-for-invalid-stream', (
        ...
        Future serverFun() async {
          ...
          expect(await nextFrame(), isA<DataFrame>().having((p0) => p0.hasEndStreamFlag, 'Last data frame', true));
          ...
          expect(
            await nextFrame(),
            isA<WindowUpdateFrame>().having((p0) => p0.header.streamId, 'Connection update', 0),
          );
          expect(
            await nextFrame(),
            isA<RstStreamFrame>()
                .having((f) => f.errorCode, 'errorCode', ErrorCode.STREAM_CLOSED)
                .having((f) => f.header.streamId, 'header.streamId', invalidStreamId),
          );

          // Close the original stream.
          serverWriter.writeDataFrame(headers.header.streamId, [], endStream: true);
      ...
      clientTest('data-frame-after-stream-closed', (
        ...
        Future serverFun() async {
          ...
          expect(
            ...
            isA<WindowUpdateFrame>()
                ...
                .having((p0) => p0.windowSizeIncrement, 'Windowsize', data1.length),
          );
          ...
          expect(
            ...
            isA<WindowUpdateFrame>()
                .having((p0) => p0.header.streamId, 'Connection update', 0)
                .having((p0) => p0.windowSizeIncrement, 'Windowsize', data2.length),
          );
        ...
        Future clientFun() async {
          ...
          expect(messages[0], isA<DataStreamMessage>().having((p0) => p0.bytes, 'Same as `data1` above', [42, 42]));
      ...
      clientTest('data-frame-received-after-stream-cancel', (
        ...
        Future clientFun() async {
          ...
          expect(message, isA<DataStreamMessage>().having((p0) => p0.bytes, 'Same sent data above', [42]));
      ...
      clientTest('client-reports-connection-error-on-push-to-nonexistent', (
        ...
        Future serverFun() async {
          ...
          // Write response.
          serverWriter.writeHeadersFrame(streamId, [Header.ascii('a', 'b')], endStream: true);

          // Push stream to the (non existing) one.
          var pushStreamId = 2;
          serverWriter.writePushPromiseFrame(streamId, pushStreamId, [Header.ascii('a', 'b')]);

          // Make sure we get a connection error.
          var frame = await nextFrame() as GoawayFrame;
          expect(ascii.decode(frame.debugData), contains('Cannot push on a non-existent stream'));
          expect(await serverReader.moveNext(), false);
          await serverWriter.close();
        }
        ...
        Future clientFun() async {
          ...
          expect(
            messages[0],
            isA<HeadersStreamMessage>().having((p0) => p0.headers.first, 'Same sent headers above', isHeader('a', 'b')),
          );
      ...
      clientTest('client-reports-connection-error-on-push-to-non-open', (
        ...
        Future serverFun() async {
          ...
          // Make sure we get a connection error.
          var frame = await nextFrame() as GoawayFrame;
          expect(ascii.decode(frame.debugData), contains('Expected open state (was: StreamState.HalfClosedRemote)'));
          expect(await serverReader.moveNext(), false);
          await serverWriter.close();
        }
    ...
    group('client-errors', () {
      clientTest('client-resets-stream', (
        ...
        Future serverFun() async {
          ...
          expect(
            await nextFrame(),
            isA<RstStreamFrame>().having((p0) => p0.errorCode, 'Stream reset', ErrorCode.CANCEL),
          );

          // Make sure we get the graceful shutdown message.
          expect(
            await nextFrame(),
            isA<GoawayFrame>().having((p0) => p0.errorCode, 'Stream reset', ErrorCode.NO_ERROR),
          );
        ...
        Future clientFun() async {
          ...
          var stream = client.makeRequest([Header.ascii('a', 'b')], endStream: false);
...
void clientTest(
  ...
  return test(name, () {
    ...
    return func(streams.clientConnection, streams.serverConnectionFrameWriter, serverReader, readNext);
  });
}
...
class ClientStreams {
  ...
  StreamIterator<Frame> get serverConnectionFrameReader {
    ...
    return StreamIterator(FrameReader(streamAfterConnectionPreface, localSettings).startDecoding());
  }
  ...
  ClientTransportConnection get clientConnection => ClientTransportConnection.viaStreams(readB, writeA);
}
