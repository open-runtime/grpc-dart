...
void main() {
  group('transport-test', () {
    transportTest('ping', (TransportConnection client, TransportConnection server) async {
      await client.ping();
      await server.ping();
    });

    transportTest('terminated-client-ping', (TransportConnection client, TransportConnection server) async {
    ...
    transportTest('terminated-server-ping', (TransportConnection client, TransportConnection server) async {
      var clientError = client.ping().catchError(
        expectAsync2((Object e, StackTrace s) {
          expect(e, isA<TransportException>());
        }),
    ...
    }, serverSettings: const ServerSettings(concurrentStreamLimit: concurrentStreamLimit));

    transportTest('disabled-push', (ClientTransportConnection client, ServerTransportConnection server) async {
      server.incomingStreams.listen(
        expectAsync1((ServerTransportStream stream) async {
          expect(stream.canPush, false);
          expect(() => stream.push([Header.ascii('a', 'b')]), throwsA(const TypeMatcher<StateError>()));
          stream.sendHeaders([Header.ascii('x', 'y')], endStream: true);
        }),
      );

      var stream = client.makeRequest([Header.ascii('a', 'b')], endStream: true);

      var messages = await stream.incomingMessages.toList();
      expect(messages, hasLength(1));
      expect(messages[0] is HeadersStreamMessage, true);
      expect((messages[0] as HeadersStreamMessage).headers[0], isHeader('x', 'y'));
    ...
    transportTest(
        ...
        Future serverFun() async {
          await for (ServerTransportStream stream in server.incomingStreams) {
            ...
            for (var pushedStream in pushes) {
              pushedStream.sendHeaders([Header.ascii('e', 'nd')], endStream: true);
          ...
          for (var i = 0; i < kRepetitions; i++) {
            ...
            Future<int> expectPeerPushes() async {
              ...
              await for (TransportStreamPush pushedStream in stream.peerPushes) {
                numberOfPushes++;
                var messages = await pushedStream.stream.incomingMessages.toList();
                expect(messages, hasLength(1));
                expect((messages[0] as HeadersStreamMessage).headers[0], isHeader('e', 'nd'));
      ...
      clientSettings: const ClientSettings(concurrentStreamLimit: kDefaultStreamLimit, allowServerPushes: true),
    );

    transportTest('early-shutdown', (ClientTransportConnection client, ServerTransportConnection server) async {
      Future serverFun() async {
        await for (ServerTransportStream stream in server.incomingStreams) {
          stream.sendHeaders([Header.ascii('x', 'y')], endStream: true);
          expect(await stream.incomingMessages.toList(), hasLength(1));
    ...
    transportTest('idle-handler', (ClientTransportConnection client, ServerTransportConnection server) async {
      Future serverFun() async {
        var activeCount = 0;
        var idleCount = 0;
        ...
        await for (final stream in server.incomingStreams) {
          stream.sendHeaders([]);
          unawaited(stream.incomingMessages.toList().then((_) => stream.outgoingMessages.close()));
        }
        await server.finish();
      ...
      Future clientFun() async {
        ...
        final streams = List<ClientTransportStream>.generate(5, (_) => client.makeRequest([]));
    ...
    group('flow-control', () {
      ...
      Future testWindowSize(
        ...
        expect(expectedStreamFlowcontrolWindow, lessThan(kChunkSize * kNumberOfMessages));
          ...
          await for (ServerTransportStream stream in server.incomingStreams) {
            ...
            controller
              ...
              ..onPause = expectAsync0(() {
                ...
                // the pause event).
                expect(serverSentBytes - kChunkSize + 1, lessThan(expectedStreamFlowcontrolWindow));
        ...
        Future clientFun() async {
          ...
          var sub = stream.incomingMessages.listen((message) {
            if (!gotHeadersFrame) {
              ...
              expect(dataMessage.bytes[0], ((byteNr ~/ kChunkSize) + (byteNr % kChunkSize)) % 256);
...
void transportTest(
  String name,
  Future<void> Function(ClientTransportConnection, ServerTransportConnection) func, {
...
class BidirectionalConnection {
  ...
  ClientTransportConnection get clientConnection =>
      ClientTransportConnection.viaStreams(readA, writeB.sink, settings: clientSettings);

  ServerTransportConnection get serverConnection =>
      ServerTransportConnection.viaStreams(readB, writeA.sink, settings: serverSettings);
}
