...
Future<void> hybridMain(StreamChannel channel) async {
  ...
  await File(config).writeAsString(envoyConfig.replaceAll('%TARGET_PORT%', server.port.toString()));
...
''');
  ...
  final portRe = RegExp(r'Set listener listener_0 socket factory local address to 0.0.0.0:(\d+)');

  proxy.stderr.transform(utf8.decoder).transform(const LineSplitter()).listen((line) {
    _info('envoy|stderr] $line');
    final m = portRe.firstMatch(line);
    if (m != null) {
      channel.sink.add({'grpcPort': int.parse(m[1]!), 'httpPort': httpServer.port});
    }
  });

  proxy.stdout.transform(utf8.decoder).transform(const LineSplitter()).listen((line) {
...
Future<HttpServer> startHttpServer() async {
  ...
  server.listen((request) async {
    ...
    final message = await GrpcHttpDecoder().bind(request.map((list) => DataStreamMessage(list))).first as GrpcData;
