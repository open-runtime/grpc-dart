...
void main() {
  group('Server lifecycle regressions', () {
    test('shutdown with active streaming handlers settles and empties map', () async {
      ...
      expect(unexpectedErrors, isEmpty, reason: 'Expected only GrpcError stream termination signals');
    ...
    test('shutdown completes even with one broken and one active connection', () async {
      final server = Server.create(services: [EchoService()]);
      await server.serve(address: 'localhost', port: 0);
      addTearDown(() => server.shutdown());
  ...
  group('Client lifecycle regressions', () {
    test('makeRequest on null transport throws unavailable instead of null crash', () async {
      ...
      expect(
        ...
        throwsA(
          isA<GrpcError>()
              ...
              .having((e) => e.message, 'message', contains('Connection not ready')),
        ),
      );
    });
  });
  ...
  group('Keepalive regressions', () {
    test('tooManyBadPings callback fires only once under ping flood', () async {
      ...
      FakeAsync().run((async) {
        ...
        expect(tooManyBadPingsCalls, equals(1), reason: 'keepalive termination callback should be idempotent');
      });
    });
  });
}
